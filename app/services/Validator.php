...
class Validator {
  ...
  public static function validateRegister(array $input, UserRepository $repo = null) {
    ...
    if (strlen($values['prenom']) < 2) $errors['prenom'] = "Le prénom doit contenir au moins 2 caractères.";
  ...
  public static function validateLogin(array $input, UserRepository $repo = null) {
    ...
    if (strlen($values['email']) < 2) $errors['email'] = "Error email";

    $ok = false;
    $user = null;

    if ($errors['email'] === '') {
        $user = $repo->getByEmail($values['email']);
        if ($user != null) {
            $ok = true;
        } else {
    ...
    return ['ok' => $ok, 'errors' => $errors, 'values' => $values, 'user' => $user];
  }
